c<-read.csv("rainfall.csv",stringsAsFactors = FALSE)
library(dplyr)
library(tseries)
library(forecast)
head(c)
c<-filter(c,SD_Name=="WEST UTTAR PRADESH")
head(c)
tail(c)
d<-filter(c,YEAR<=2013)
c<-filter(c,YEAR<=2012)
library(reshape)
str(c)
c<-select(c,YEAR,JAN:APR)
d<-select(d,YEAR,JAN:APR)
mel<-melt(c,c("YEAR"))
mel1<-melt(d,c("YEAR"))
head(mel)
tail(mel1)
mel<-arrange(mel,YEAR)
mel1<-arrange(mel1,YEAR)
head(mel,12)
mel<-ts(mel$value,frequency=4)
mel1<-ts(mel1$value,frequency=4)
class(mel)
class(mel1)
plot(decompose(mel))
x<-decompose(mel)$trend
x1<-decompose(mel1)$trend
y<-decompose(mel)$seasonal
z<-decompose(mel)$random
x<-na.omit(x)
x1<-na.omit(x1)
z<-na.omit(z)
#t<-na.omit(x)
#t1<-na.omit(x1)
#x<-ifelse(is.na(x),median(t),x)
#x1<-ifelse(is.na(x1),median(t1),x)
#t<-na.omit(z)
#z<-ifelse(is.na(z),median(t),z)
x<-ts(x,frequency=4)
x1<-ts(x1,frequency = 4)
y<-ts(y,frequency=4)
z<-ts(z,frequency =4)
auto.arima(x)
model<-Arima(x,order=c(3,0,0),seasonal=list(order=c(2,0,0),period=4))
model2=auto.arima(y)
model1<-Arima(y,order=c(2,1,0))
model3=auto.arima(z)
model2<-Arima(z,order=c(3,0,0),seasonal=list(order=c(2,0,0),period=4))
library(forecast)
pre<-forecast(model,h=4)
pre1<-forecast(model2,h=4)
pre2<-forecast(model3,h=4)
pre<-as.data.frame(pre)
seas<-head(y,4)
seas<-as.data.frame(seas)
pre2<-as.data.frame(pre2)
s<-seas$x+pre$`Point Forecast`+pre2$`Point Forecast`
#plot(s)
#lines(x1,col='red')
d<-filter(d,YEAR==2013)
d<-melt(d,c("YEAR"))
d<-ts(d$value,frequency=4)
s<-ifelse(s>0,s,0)
plot(d,col='red')
lines(s)
